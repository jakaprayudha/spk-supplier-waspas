...
if(isset($_POST['submit'])):	
	...
	$ada_pilihan = trim($_POST['ada_pilihan']);
	...
	if(empty($errors)):
		...
		// Prepare and bind
		$stmt = $koneksi->prepare("INSERT INTO kriteria (kode_kriteria, nama, type, bobot, ada_pilihan) VALUES (?, ?, ?, ?, ?)");
		if (!$stmt) {
			$errors[] = 'Query preparation failed: ' . $koneksi->error;
		} else {
			$stmt->bind_param("sssss", $kode_kriteria, $nama, $type, $bobot, $ada_pilihan);
